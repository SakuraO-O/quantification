...
def get_csi_index_data(code):
    try:
        df = fetch_with_retry(ak.index_zh_a_hist, symbol=code, period="daily", start_date="20150101")
        return df
    except:
        return None
...
def get_hk_index_data(code):
    try:
        if code == "HSI":
            df = fetch_with_retry(ak.index_hk_spot_em)
            hsi_data = df[df["代码"] == "HSI"]
            if len(hsi_data) > 0:
                current_price = hsi_data.iloc[0]["最新价"]
                hist_df = fetch_with_retry(ak.index_hk_hist, symbol="HSI", period="daily", start_date="20150101")
                return hist_df
        elif code == "HSTECH":
            df = fetch_with_retry(ak.index_hk_spot_em)
            hstech_data = df[df["代码"] == "HSTECH"]
            if len(hstech_data) > 0:
                current_price = hstech_data.iloc[0]["最新价"]
                hist_df = fetch_with_retry(ak.index_hk_hist, symbol="HSTECH", period="daily", start_date="20150101")
                return hist_df
        return None
    except:
        return None
...
def get_index_pe(code, market):
    try:
        if market == "csi":
            df = fetch_with_retry(ak.index_value_hist_funddb, symbol=code, indicator="市盈率")
            return df
        else:
            return None
    except:
